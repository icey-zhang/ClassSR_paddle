...
#import paddle.DataParallel as DataParallel
...
class ClassSR_Model(BaseModel): #in
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if loss_type == 'l1':
                ...
                self.cri_pix = nn.L1Loss()
                self.class_loss = class_loss_3class()
                self.average_loss = average_loss_3class()
            else:
            ...
            if train_opt['lr_scheme'] == 'MultiStepLR':
                ...
                # for optimizer in self.optimizers:
                self.schedulers=paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=train_opt['lr_G'], T_max=train_opt['T_period'][0],eta_min=train_opt['eta_min'])
    ...
    def test(self):
        ...
        for LR_img,GT_img in zip(lr_list,gt_list):
            ...
            if img.shape[2] > 3:
                img = img[:, :, :3]
            img = img[:, :, [2, 1, 0]]
            img = paddle.to_tensor(np.expand_dims(np.ascontiguousarray(np.transpose(img, (2, 0, 1))), axis=0),dtype='float32')# .float() .to(self.device) 
            with paddle.no_grad():
                srt, type = self.netG(img, False)

            if self.which_model == 'classSR_3class_rcan':
                sr_img = util.tensor2img(srt.detach()[0].cpu(), out_type=np.uint8, min_max=(0, 255)) #.float()
            else:
                sr_img = util.tensor2img(srt.detach()[0].cpu()) #.float()
            sr_list.append(sr_img)

            if index == 0:
                type_res = type
            else:
                type_res = paddle.concat((type_res, type), axis=0)

            psnr=util.calculate_psnr(sr_img, GT_img)
            flag=paddle.argmax(type, 1).numpy()
            if flag == 0:
                psnr_type1 += psnr
            if flag == 1:
                psnr_type2 += psnr
    ...
    def combine_addmask(self, sr_list, num_h, num_w, h, w, patch_size, step, type):
        ...
        for i in range(num_h):
            for j in range(num_w):
                ...
                if paddle.argmax(type, 1).numpy()[index2] == 0:
                ...
                elif paddle.argmax(type, 1).numpy()[index2] == 1:
                    # mask1 = cv2.rectangle(zeros1, (bbox1[0], bbox1[1]), (bbox1[2], bbox1[3]),
                    #                       color=(0, 0, 0), thickness=1)
                    mask2 = cv2.rectangle(zeros1, (bbox1[0]+1, bbox1[1]+1), (bbox1[2]-1, bbox1[3]-1),
                                            color=(0, 255, 255), thickness=-1)# medium yellow
                elif paddle.argmax(type, 1).numpy()[index2] == 2:
                    # mask1 = cv2.rectangle(zeros1, (bbox1[0], bbox1[1]), (bbox1[2], bbox1[3]),
                    #                       color=(0, 0, 0), thickness=1)
                    mask2 = cv2.rectangle(zeros1, (bbox1[0]+1, bbox1[1]+1), (bbox1[2]-1, bbox1[3]-1),
    ...
    def print_res(self, type_res):
        ...
        for i in paddle.argmax(type_res, 1).numpy():
            if i == 0:
                num0 += 1
            if i == 1:
                num1 += 1
