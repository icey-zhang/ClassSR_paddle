...
def initialize_weights(net_l, scale=1): #use
...
def initialize_weights_nonSequential(net_l, scale=1): #use
    if not isinstance(net_l, list):
        net_l = [net_l]
    for m in net_l:
        # for m in net:
...
#         for m in net:
#             if isinstance(m, nn.Conv2D):
#                 m.weight.weight_attr =init.KaimingNormal()
#                 if m.bias is not None:
#                     m.weight.bias_attr = init.Constant(value=0)
...
#                 m.weight.weight_attr = init.Constant(value=0)
#                 m.weight.bias_attr = init.Constant(value=0)
...
# rcan arch
def default_conv(in_channels, out_channels, kernel_size, weight_attr =init.KaimingNormal(), bias_attr = init.Constant(value=0.)): #use
    return nn.Conv2D(
        in_channels, out_channels, kernel_size,
        padding=(kernel_size//2),weight_attr=weight_attr, bias_attr=bias_attr)
def default_conv_for_Classifier(in_channels, out_channels, kernel_size, stride=1, padding=0, weight_attr =init.KaimingNormal(), bias_attr = init.Constant(value=0.)): #use
    return nn.Conv2D(
        in_channels, out_channels, kernel_size,stride=stride,
        padding=padding,weight_attr=weight_attr, bias_attr=bias_attr)
class MeanShift(nn.Conv2D): #use 有问题
    def __init__(self, rgb_range, rgb_mean, rgb_std, sign=-1):
        super(MeanShift, self).__init__(3, 3, kernel_size=1)
        # std = paddle.to_tensor(rgb_std)
        # self.weight.data = paddle.eye(3).reshape([3, 3, 1, 1])
        # self.weight.data=paddle.divide(self.weight,std.reshape([3, 1, 1, 1]))
        # self.bias.data = sign * rgb_range * paddle.to_tensor(rgb_mean)
        # self.bias.data=paddle.divide(self.weight,std)
...
#         bias_attr=None, bn=False, act=nn.ReLU(True), res_scale=1):
...
#     vgrid_x = 2.0 * vgrid[:, :, :, 0] / max(W - 1, 1) - 1.0
#     vgrid_y = 2.0 * vgrid[:, :, :, 1] / max(H - 1, 1) - 1.0
#     vgrid_scaled = paddle.stack((vgrid_x, vgrid_y), dim=3)
#     output = F.grid_sample(x, vgrid_scaled, mode=interp_mode, padding_mode=padding_mode)
#     return output
